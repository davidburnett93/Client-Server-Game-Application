#David Burnett
#251179445
#This class acts as a room in the game, storing the room data and executing actions of the room
import argparse
import signal
import socket
import sys
...
def signal_handler(sig, frame):
    ...
    #Before the server shuts down, deregister through the discovery class
    deregister = 'DEREGISTER ' + name
    room_socket.sendto(deregister.encode(), (DISCOVERYHOST,DISCOVERYPORT))
    ...
    # drop them from the server as they're going there next, and tell everyone else.
        ...
        if (connections.get(words[0]) == ""):
            ...
            for player in players:
                ...
                room_socket.sendto(message.encode(),client_addr)
            client_remove(client_search_by_address(addr))
            #return the room name in the specified direction            
...
def main():
    ...
    # the port allocated to the server is decided by the OS

    room_socket.bind(('', 0))

    #Regeister the room with discovery
    register = 'REGISTER room://localhost:' + str(room_socket.getsockname()[1]) + ' ' + name
    room_socket.sendto(register.encode(), (DISCOVERYHOST,DISCOVERYPORT))
    response, addr = room_socket.recvfrom(1024)
    #If the room is registered
    if response.decode() == 'OK':
